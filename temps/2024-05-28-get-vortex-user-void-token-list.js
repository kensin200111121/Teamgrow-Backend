...
const { getAllTokens } = require('../src/services/identity');
...
const getList = async (NODE_ENV) => {
  ...
  for (let i = 0; i < workIds.length; i++) {
    const idInfo = workIds[i];
    console.log('----', i, ', ', idInfo);
    const identityTokens = await getAllTokens({
      userId: idInfo.vortexId,
      query: 'omitDetails=true',
    }).catch((error) => {
      console.log('identity tokens get error', error.message || error.msg);
    });
    console.log('-----identityTokens', identityTokens);
    let user = null;
    if (!identityTokens?.length) {
      user = await User.findOne({
        _id: mongoose.Types.ObjectId(idInfo.id),
      });
    }
    invalidUserList.push({
      id: idInfo.id,
      vortexId: idInfo.vortexId,
      email: user?.email,
    });
  }
  const useremail = fs.createWriteStream('userList.csv');
  useremail.on('error', (err) => {
    console.log('error');
  });
  invalidUserList.forEach((v) => {
    useremail.write(v.id + ', ' + v.vortexId + ', ' + v.email + ', \n');
  });
  useremail.end();
};
