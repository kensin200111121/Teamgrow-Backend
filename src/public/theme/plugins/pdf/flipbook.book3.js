...
FLIPBOOK.Book3.prototype.isZoomed = function () {
  return (
    this.options.main.zoom > this.options.zoomMin && this.options.main.zoom > 1
  );
};
...
FLIPBOOK.Book3.prototype.focusLeft = function (time, delay) {
  var pos =
    this.view == 1 || this.options.singlePageMode ? 0 : this.pageWidth / 2;
...
FLIPBOOK.Book3.prototype.focusRight = function (time, delay) {
  var pos =
    this.view == 1 || this.options.singlePageMode
      ? -this.pageWidth
      : -this.pageWidth / 2;
...
FLIPBOOK.Book3.prototype.setBookPosition = function (pos, time, delay) {
  if (this.centerContainerPosition == pos) return;
    ...
    jQuery({
      ...
      .animate(
        ...
        },
        {
          ...
          step: function (now) {
            self._setStyle(
              self.centerContainerStyle,
              FLIPBOOK.IScroll.utils.style.transform,
              'translateX(' + now + 'px) translateZ(0)'
            );
          },
          complete: function () {
...
FLIPBOOK.Book3.prototype.updateVisiblePages = function (loadNextPrev) {
  if (typeof loadNextPrev == 'undefined') loadNextPrev = true;
  var self = this;
...
FLIPBOOK.Book3.prototype.prevPage = function () {
  ...
  var target = this.options.singlePageMode
    ? this.rightIndex
    : this.rightIndex - 2;
...
FLIPBOOK.Book3.prototype.goToPage = function (index, instant) {
  ...
  if (!this.options.singlePageMode) {
    if (this.view == 1) {
      if (index < this.rightIndex) this.focusRight(d);
      else this.focusLeft(d);
    } else {
      if (index == 0) this.focusRight(d);
      else if (index == this.pagesArr.length) this.focusLeft(d);
      else this.focusBoth(d);
    }
  }
  ...
  if (this.options.singlePageMode) d /= 2;
  jQuery({
...
FLIPBOOK.Book3.prototype.onSwipe = function (
  event,
  phase,
  direction,
  distance,
  ...
  if (event.target.className == 'flipbook-page-link') return;
  ...
  if (phase == 'cancel' && fingerCount <= 1) {
    if (angle > 0) this.nextPage();
    else if (angle < 0) this.prevPage();
  }
  ...
  if (phase == 'move' && fingerCount <= 1) {
    if (angle > 0) {
      if (!this.nextEnabled) return;
      if (this.options.singlePageMode) {
        if (this.rightIndex == this.pagesArr.length - 1) return;
      }
    ...
    if (
      ...
      if (!this.options.instantFlip) this._setPageAngle(angle);
  }
};
...
FLIPBOOK.Book3.prototype._setPageAngle = function (angle) {
  ...
  if (angle > 0) {
    if (this.view == 1 && this.isFocusedLeft()) return;
    ...
    if (this.view == 1 && this.isFocusedRight()) return;
...
FLIPBOOK.Book3.prototype.onPageUnloaded = function (index, size) {
  ...
  if (this.pagesArr[pageIndex]) this.pagesArr[pageIndex].unload();
};
...
FLIPBOOK.Book3.prototype.onResize = function () {
  ...
  function fit() {
    if (self.scroll) self.scroll.options.zoomMin = self.ratio * options.zoomMin;
    if (self.scroll) self.scroll.options.zoomMax = self.ratio * options.zoomMax;
...
FLIPBOOK.Book3.prototype.isFocusedRight = function () {
  ...
  if (this.options.singlePageMode) return this.rightIndex % 2 == 0;
  else return this.centerContainerPosition < center;
};

FLIPBOOK.Book3.prototype.isFocusedLeft = function () {
  var center = this.view == 1 ? -this.pageWidth / 2 : 0;

  if (this.options.singlePageMode) return this.rightIndex % 2 == 1;
  else return this.centerContainerPosition > center;
};
...
FLIPBOOK.Page3 = function (book, index, texture, html) {
  ...
  this.$wrapper = jQuery('<div>')
    ...
    .width(book.options.pageWidth)
    .height(book.options.pageHeight);
  ...
  if (options.pagePreloader)
    this.$preloader = jQuery(
      '<img src="' +
        options.pagePreloader +
        '" class="flipbook-page-preloader-image">'
    ).appendTo(this.$wrapper);
  else
    this.$preloader = jQuery(
      '<img src="' +
        options.assets.spinner +
...
FLIPBOOK.Page3.prototype = {
  load: function (callback) {
    ...
    this.options.main.loadPageHTML(index, function (c) {
      ...
      if (c) {
        ...
        self.$html[0].style.transform =
          'scale(' + self.htmlContentScale + ') translateZ(0)';
      }
    ...
    this.options.main.loadPage(
      ...
      function (page) {
        if (page.image) {
          ...
          if (
            self.options.pages[page.index].side == 'left' ||
            self.options.pages[page.index].side == 'right'
          ) {
            if (self.index % 2 == 0)
              jQuery(img).css(
                FLIPBOOK.IScroll.utils.style.transform,
        ...
        if (page.canvas && self.pageSize != page.size) {
          ...
          } else jQuery(page.canvas).appendTo(self.$bg);
  ...
  setDataUrl: function (dataUrl, h) {
    ...
    if (self.html) jQuery(self.html).appendTo(self.$inner);
  },
  ...
  _setAngle: function (angle) {
    ...
    if (this.book.options.viewMode == '3d') {
      ...
      if (this.type == 'front') {
        ...
        transformStyle =
          'scaleX(' + String((180 - 2 * angle) / 180) + ') ' + suffix;
      } else {
        if (angle < 90) angle = 90;
        transformStyle =
          'scaleX(' + String(-1 + (2 * angle) / 180) + ') ' + suffix;
      }
    }
  ...
  _setZIndex: function (val) {
    if (this.zIndex != val) this.wrapper.style['z-index'] = val;
    this.zIndex = val;
  },
