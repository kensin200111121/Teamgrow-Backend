...
var isLoading = false;
var track_id;
const trackPDF = (pageNumber = 1) => {
  if (isLoading) {
    return;
  }
  if (isLeadMode && (!isCaptured || !activity)) {
    return;
  }
  if (isLoaded && isOpened) {
    if (!isRegistered) {
      isRegistered = true;
      isLoading = true;
      let api = 'api/material/track-pdf';
      if (extension == 'true') {
        api = 'api/material/extension-track-pdf';
      }
      $.ajax({
        type: 'POST',
        url: api,
        ...
        data: JSON.stringify({
          ...
          total_pages: PDFViewerApplication.pageCount,
        }),
      ...
      isLoading = true;
      let api = 'api/material/track-pdf-update/';
      if (extension == 'true') {
        api = 'api/material/extension-track-pdf-update/';
      }
      $.ajax({
        type: 'PUT',
        url: api + track_id,
        ...
        data: JSON.stringify({ read_pages: pageNumber }),
        success: () => {
          isLoading = false;
        },
        error: () => {
...
const pdfViewer = document.querySelector('#pdf-viewer-wrapper');
if (pdfViewer) {
  pdfViewer.innerHTML = `
...
`;
  ...
  const pdfTrigger = document.querySelector('#pdf-container');
  pdfTrigger.addEventListener('click', () => {
    pdfViewer.classList.add('opened');
    isOpened = true;
    trackPDF();
  });

  const closeReaderBtn = document.querySelector('#closeReader');
  closeReaderBtn.addEventListener('click', () => {
    pdfViewer.classList.remove('opened');
  });
}

window['pdf_file_path'] = pdfViewer.dataset.pdf;
  ...
  /******/ [
    ...
    /***/ function (module, exports, __webpack_require__) {
      ...
      function getViewerConfiguration() {
        return {
          appContainer: document.getElementById('pdf-viewer-wrapper'),
    ...
    /***/ function (module, exports, __webpack_require__) {
      ...
      function scrollIntoView(element, spot) {
        console.log('element', element, element.offsetParent);
      ...
      function watchScroll(viewAreaElement, callback) {
        ...
        console.log('viewAreaElement', viewAreaElement);
        var state = {
          right: true,
          down: true,
          lastX: viewAreaElement && viewAreaElement.scrollLeft,
          lastY: viewAreaElement && viewAreaElement.scrollTop,
      ...
      function waitOnEventOrTimeout(_ref2) {
        ...
        return new Promise(function (resolve, reject) {
          ...
          if (target instanceof EventBus) {
            ...
            target && target.addEventListener(name, eventHandler);
          }
    ...
    /***/ function (module, exports, __webpack_require__) {
      ...
      var defaultOptions = {
        ...
        workerSrc: {
          value: './theme/plugins/pdf/pdf.worker.js',
          kind: OptionKind.WORKER,
        },
      };
      {
