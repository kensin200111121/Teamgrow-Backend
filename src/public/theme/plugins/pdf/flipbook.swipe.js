...
FLIPBOOK.BookSwipe = function (el, wrapper, model, options) {
  ...
  for (var i = 0; i < options.numPages; i++) {
    ...
    if (options.loadAllPages) page.load();
  }
  ...
  this.iscroll.on('scrollEnd', function () {
    ...
    if (self.currentSlide == sliderPage) return;
...
FLIPBOOK.BookSwipe.prototype.setSlidePages = function (slide, pages) {
  ...
  if (this.slides[slide].iscroll) this.slides[slide].iscroll.refresh();
...
FLIPBOOK.BookSwipe.prototype.setZoomPages = function (pages) {
  if (
    ...
    this.$zoomScroller.pages.join('') == pages.join('')
  )
    return;
...
FLIPBOOK.BookSwipe.prototype.getCurrentSlidePages = function () {
  if (this.singlePage) return [this.rightIndex];
  else return [this.rightIndex - 1, this.rightIndex];
};
...
FLIPBOOK.BookSwipe.prototype.updateVisiblePages = function () {
  ...
  if (this.singlePage) {
    ...
    if (!left) {
      ...
      if (afterNext) this.setSlidePages(1, [next, afterNext]);
      else this.setSlidePages(1, [next]);
      ...
      if (right) {
        ...
        if (!next) {
          ...
          if (beforePrev) this.setSlidePages(1, [beforePrev, prev]);
          else this.setSlidePages(1, [prev]);
          ...
          if (prev) {
            ...
            if (beforePrev) this.setSlidePages(0, [beforePrev, prev]);
            else this.setSlidePages(0, [prev]);

            if (afterNext) this.setSlidePages(2, [next, afterNext]);
            else this.setSlidePages(2, [next]);
...
FLIPBOOK.BookSwipe.prototype.onResize = function () {
  ...
  this.$scroller.width(
    this.$scroller.children().length * (w + spaceBetweenSlides)
  );
...
FLIPBOOK.BookSwipe.prototype.resizeInnerSlide = function (slide) {
  ...
  else this.slides[slide].find('.slide-inner').width(pw);
};
...
FLIPBOOK.BookSwipe.prototype.goToSlide = function (slideIndex, instant) {
  ...
  if (this.iscroll.pages.length > 0) this.iscroll.goToPage(slideIndex, 0, time);

  if (instant) this.currentSlide = slideIndex;
...
FLIPBOOK.BookSwipe.prototype.zoomTo = function (zoom, time, x, y) {
  ...
  for (var i = 0; i < 3; i++) {
    if (this.slides[i].iscroll) {
      if (scale < this.slides[i].iscroll.options.zoomMin)
        scale = this.slides[i].iscroll.options.zoomMin;
...
FLIPBOOK.BookSwipe.prototype.onSwipe = function (
  event,
  phase,
  direction,
  distance,
...
FLIPBOOK.BookSwipe.prototype.enablePan = function () {
  if (this.slides[0].iscroll) this.slides[0].iscroll.enable();
  if (this.slides[1].iscroll) this.slides[1].iscroll.enable();
  if (this.slides[2].iscroll) this.slides[2].iscroll.enable();
};

FLIPBOOK.BookSwipe.prototype.disablePan = function () {
  if (this.slides[0].iscroll) this.slides[0].iscroll.disable();
  if (this.slides[1].iscroll) this.slides[1].iscroll.disable();
  if (this.slides[2].iscroll) this.slides[2].iscroll.disable();
};
...
FLIPBOOK.PageSwipe.prototype = {
          ...
          if (self.options.doublePage) {
            if (
              ...
              (self.index == self.options.pages.length - 1 &&
                !self.options.oddPages)
            )
              self.$img.css('width', '100%');
            else self.$img.css('width', '200%');
          } else self.$img.css('width', '100%');
  ...
  setSize: function () {
    var h = this.options.pageTextureSize,
      w;
  ...
  setRotation: function (val) {
    ...
    if (typeof val != 'undefined') this.rotation = val;
    if (this.$img) {
      this.$img.css(
        'transform',
