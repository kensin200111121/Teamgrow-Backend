...

{
  ...
  FLIPBOOK.PageWebGL = function (
    ...
    var index = options.rightToLeft
      ? options.pages.length / 2 - this.index - 1
      : this.index;
...

{
  ...
  FLIPBOOK.BookWebGL.prototype.goToPage = function (index, instant, moved) {
    ...
    if (instant) {
      for (var i = 0; i < this.pages.length; i++) {
        if (this.pages[i].flippingLeft || this.pages[i].flippingRight) return;
      }
    }
    ...
    if (this.view == 1 && !moved) {
      ...
      if (index % 2 == 0) this.focusLeft(time);
      else this.focusRight(time);
    }
    ...
    if (this.rightIndex > index) {
      ...
      if (this.rightIndex - 2 > index) {
        ...
        else self.goToPage(index, instant, 1);
      } else {
        this.prevPage();
        setTimeout(function () {
          if (typeof instant != 'undefined' && instant) {
  ...
  FLIPBOOK.BookWebGL.prototype.updateBookPosition = function () {
    if (this.view == 1) {
      if (this.flippedright == 0) this.focusLeft();
      else if (this.flippedleft == 0) this.focusRight();
      else this.isFocusedLeft() ? this.focusLeft() : this.focusRight();
    } else {
  ...
  FLIPBOOK.BookWebGL.prototype.moveToPos = function (
    pos,
    time,
    delay,
    callback
  ...
  FLIPBOOK.BookWebGL.prototype.move = function (direction) {
    ...
    this.centerContainer.position.x +=
      (10000 * posX) / (this.cameraZ * this.zoom * this.zoom);
    this.centerContainer.position.y -=
      (10000 * posY) / (this.cameraZ * this.zoom * this.zoom);
    this.updateHtmlLayerPosition();
  };
