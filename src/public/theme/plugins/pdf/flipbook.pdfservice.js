...
FLIPBOOK.PdfService = function (pdfDocument, model, options) {
  ...
  this.isRendering = function (pi, size) {
    ...
    this.canvasBuffer.forEach(function (c) {
      if (c.size == size && c.pdfPageIndex == pi && c.rendering) val = true;
    });
    return val;
  };
  ...
  this.isRendered = function (pi, size) {
    var val = false;
    this.canvasBuffer.forEach(function (c) {
      if (c.size == size && c.pdfPageIndex == pi && c.rendered) val = true;
    });
    return val;
  };
  ...
  this.setRightIndex = function (ri) {
    ...
    this.canvasBuffer.forEach(function (c) {
      var canDelete =
        (!c.rendering && ri - c.pageIndex > 1 + d) || ri - c.pageIndex < -d;
  ...
  this.loadThumbs = function (convertToBlob, callback) {
    ...
    if (this.thumbLoading >= this.pdfInfo.numPages) callback.call(self);
    else
  ...
  this.getViewport = function (index, callback) {
    if (index >= self.pdfInfo.numPages) return;
    if (!self.pages[index]) {
  ...
  this.findInPageTextContentAvailable = function (page, index) {
    ...
    var matches = page.textContentString
      .toUpperCase()
  ...
  this.getThumb = function (index, size, callback) {
    this.getViewport(index, function (viewport) {
      var page = self.pages[index];
      if (page.thumb) callback.call(self, page.thumb);
      else {
        //render thumb first
  ...
  this.getPage = function (index, callback) {
    ...
    if (pdfPageIndex > this.pdfInfo.numPages) return;
  ...
  this.renderPage = function (page, size, callback) {
    ...
    if (page.canvas[size] && page.canvas[size].rendered) {
      if (callback) callback.call(self, page);
      callback = null;
      return;
    }
  ...
  this.renderBookPage = function (bookPageIndex, size, callback) {
    var pageIndex = this.options.doublePage
      ? Math.round(bookPageIndex / 2)
      : bookPageIndex;
  ...
  this.loadPageTextLayer = function (pageIndex, bookPageIndex, callback) {
    ...
    if (pageIndex >= this.pdfInfo.numPages) callback.call(self);
    ...
    if (
      page.annotationsLoading ||
      page.textContentLoading ||
      !page.annotationsLoaded ||
      !page.textContentLoaded
    ) {
      setTimeout(function () {
        self.loadPageTextLayer(pageIndex, bookPageIndex, callback);
      }, 100);
  ...
  this.loadTextLayer = function (bookPageIndex, callback) {
    ...
    this.loadPageTextLayer(
      pageIndex,
      bookPageIndex,
      function (pdfPage, bookPageIndex) {
        ...
        if (!page.htmlContentInitialized) {
          ...
          textLayerDiv.style.width =
            String(
              (1000 * pdfPage.getViewport({ scale: 1 }).width) /
                pdfPage.getViewport({ scale: 1 }).height
            ) + 'px';
          ...
          if (pdfPage.annotations.length > 0) {
            ...
            var parameters = {
              viewport: pdfPage
                .getViewport({
                  scale: 1000 / pdfPage.getViewport({ scale: 1 }).height,
                })
                .clone({ dontFlip: true }),
              div: div,
  ...
  this.getBookPage = function (index, size) {
    var toReturn = null;
    this.canvasBuffer.forEach(function (c) {
      if (c.pageIndex == index && c.size == size) toReturn = c;
    });
    ...
    if (options.doublePage) {
      if (pdfPageIndex == 0) {
        ...
        if (self.webgl) {
          ...
          var loadedIndexR = self.options.rightToLeft
            ? 2 * pdfPageIndex - 1
            : 2 * pdfPageIndex;
          var loadedIndexL = self.options.rightToLeft
            ? 2 * pdfPageIndex
            : 2 * pdfPageIndex - 1;
  ...
  this.getTextContent = function (page, callback) {
    ...
    if (page.textContent) page.getTextCallback(page);
    else {
...
var PDFLinkService = (function () {
  function PDFLinkService() {
    var _ref =
      ...
      externalLinkTarget =
        _ref$externalLinkTarg === undefined ? null : _ref$externalLinkTarg,
