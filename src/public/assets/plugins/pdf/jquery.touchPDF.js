  ...
  function TouchPDF(element, options) {
    ...
    function initDom() {
      ...
      if (options.showToolbar) {
        $element
          ...
          .html(
            '<div class="pdf-title">' +
              ...
              (options.disableZoom
                ? ''
                : '<div class="pdf-button"><button class="pdf-zoomin">+</button></div>' +
        ...
        $element
          .find('.pdf-toolbar > .pdf-button > .pdf-prev')
          .on('click', function () {
            goto(pageNum - 1);
          });
        $element
          .find('.pdf-toolbar > .pdf-button > .pdf-next')
          .on('click', function () {
            goto(pageNum + 1);
          });
      }
      ...
      if (!options.disableZoom) {
        ...
        if (options.showToolbar) {
          $element
            .find('.pdf-toolbar > .pdf-button > .pdf-zoomin')
            .on('click', function () {
              zoomIn();
            });
          $element
            .find('.pdf-toolbar > .pdf-button > .pdf-zoomout')
            .on('click', function () {
              zoomOut();
            });
        }
    ...
    function load() {
      ...
      if (options.tabs && $.isArray(options.tabs)) {
        ...
        $.each(options.tabs, function (i, tab) {
          ...
          if (tab.bottom === undefined)
            top[offset] += $a.height() + TAB_SPACING;
        });
      }
    }
    ...
    function renderPage() {
      ...
      pdfDoc.getPage(pageNumRendering).then(function (page) {
        ...
        $('.pdf-canvas')
          .css('transform', 'scale(' + 1 / options.quality + ')')
    ...
    function redraw() {
      ...
      scale = Math.min(
        winHeight / (pdfHeight + TOOLBAR_HEIGHT + BORDER_WIDTH * 2),
        winWidth / (pdfWidth + tabWidth * 2 + BORDER_WIDTH * 2)
      );
      if (scale > 1) scale = 1;
      ...
      $element
        ...
        .css(
          'left',
          (winWidth - scale * (pdfWidth + tabWidth * 2 + BORDER_WIDTH * 2)) / 2
        );
    ...
    function renderAnnotations(page, viewport) {
      ...
      page.getAnnotations().then(function (annotationsData) {
        ...
        $.each(annotationsData, function (i, data) {
          ...
          var $el = $(
            PDFJS.AnnotationUtils.getHtmlElement(data, page.commonObjs)
          );
          var rect = data.rect;
          var view = page.view;
          ...
          var link = $el.find('a').on('mousedown', function (e) {
            e.preventDefault();
          });
            ...
            link
              ...
              .on('click', function (e) {
                ...
                if (dest instanceof Array) {
                  ...
                  pdfDoc
                    .getDestination($(this).data('dest'))
                    .then(function (destRefs) {
